...
export function Lesson(props: LessonProps) {
    ...
    return (
        <Link to={`/event/lesson/${props.slug}`}>
            ...
            `${if(isActiveLesson) {
                 <div className={`rounded border border-gray-500 p-4 mt-2 bg-green-500 text text-white`}>
                 <header className="flex items-center justify-between">
                     
                     {isLessonAvailable ? (<span className="text-sm text-white font-medium flex items-center gap-2">
                         <CheckCircle size={20} />
                         Conteúdo liberado
                     </span>) : (<span className="text-sm text-white font-medium flex items-center gap-2">
                         <Lock size={20} />
                         Em Breve
                     </span>) }
